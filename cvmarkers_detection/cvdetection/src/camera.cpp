...
#define CAMERA_OK 1
#define CAMERA_NOT_OK 0

#define CAMERA_VERBOSE

#if defined(CAMERA_VERBOSE)
#define VERBPRINT(...) fprintf(stdout, (__VA_ARGS__))
#define VERBERR(...) fprintf(stderr, (__VA_ARGS__))
#else
#define VERBPRINT(s)
#define VERBERR(s)
#endif
...
namespace cvdetection
{
    ...
    int Camera::calibrate(const string &settingsPath, const int &winSize, const float &gridWith)
    {
        Settings s;
        ...
        if (!fs.isOpened())
        {
            VERBERR("Could not open the configuration file: \"%s\"\n");
        ...
        if (!s.goodInput)
        {
            VERBERR("Invalid input detected. Application stopping.\n");
            return CAMERA_NOT_OK;
        }
        ...
        for (;;)
        {
            ...
            {
                cout << "Press 'g' to redo calibration, 'u' to show undistorted and esc to exit." << endl;
                Mat temp = view.clone();
                if (s.useFisheye)
                {
                    Mat newCamMat;
